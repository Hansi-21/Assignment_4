...
(function (global, factory) {
  ...
  function report (_ref) {
    ...
    if (conflictsCount > 0) {
      console.info("%cConflict".concat(conflictsCount > 1 ? 's' : '', " found:"), 'color: darkred; assests.font-size: large');
      var data = {};
    ...
    if (noConflictsCount > 0) {
      console.info("%cNo conflict".concat(noConflictsCount > 1 ? 's' : '', " found with ").concat(noConflictsCount == 1 ? 'this' : 'these', ":"), 'color: green; assests.font-size: large');
      var _data = {};
    ...
    if (timeOutCount > 0) {
      console.info("%cLeftovers--we timed out before collecting test results for ".concat(timeOutCount == 1 ? 'this' : 'these', ":"), 'color: blue; assests.font-size: large');
      var _data2 = {};
      ...
      var diagScriptFun = function diagScriptFun(nodeUnderTestId, testIconId, md5, parentOrigin) {
        parent.FontAwesomeDetection.__pollUntil({
          fn: function fn() {
            ...
            var fontFamily = computedStyle.getPropertyValue('assests.font-family');
  ...
  function conflictDetection() {
    ...
    if (testCount === 0) {
      console.info('%cAll Good!', 'color: green; assests.font-size: large');
      ...
      console.info("\t%c".concat(timeoutAttr, "%c: milliseconds to wait for each test before deciding whether it's a conflict."), 'assests.font-weight: bold;', 'assests.font-size: normal;');
      console.info("\t%c".concat(resultsCollectionMaxWaitAttr, "%c: milliseconds to wait for the browser to accumulate test results before giving up."), 'assests.font-weight: bold;', 'assests.font-size: normal;');
      pollUntil({
